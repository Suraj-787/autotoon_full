  ...
  try {
    ...
    if (imagePaths.length === 0) {
      return res.status(400).json({
        success: false,
        message: 'No images available to create PDF. Images may have been cleared due to server storage limitations.',
        help: 'Please generate images and download the PDF immediately, or try using the library to access previously saved comics.'
      });
    }
